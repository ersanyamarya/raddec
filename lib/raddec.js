...
class Raddec {
  ...
  constructor(source) {
    if (Buffer.isBuffer(source)) {
      ...
      logger.error({
        message: "Raddec cannot be constructed from unhandled source",
        context: { source },
      });
  ...
  get signature() {
    logger.info({
      ...
      context:
        this.transmitterId +
        identifiers.SIGNATURE_SEPARATOR +
  ...
  get receiverSignature() {
    if (
      this.rssiSignature.length === 0 ||
      this.rssiSignature[0].receiverIdType === identifiers.TYPE_UNKNOWN
    ) {
      logger.debug({
        message: "receiverSignature - unknown",
      });
      return (
        "unknown" + identifiers.SIGNATURE_SEPARATOR + identifiers.TYPE_UNKNOWN
      );
    }
    logger.debug({
      message: "receiverSignature - known",
      context:
        this.rssiSignature[0].receiverId +
        identifiers.SIGNATURE_SEPARATOR +
  ...
  addDecoding(decoding) {
    let target = null;
    logger.debug({
      message: "addDecoding - before",
      context: { decoding, rssiSignature: this.rssiSignature },
    });
    decoding.receiverId = identifiers.format(decoding.receiverId);

    this.rssiSignature.forEach(function (current) {
      let isTarget = identifiers.areMatch(
        current.receiverId,
    ...
    if (
      decoding.hasOwnProperty("timestamp") &&
    ...
    logger.info({
      message: "addDecoding - after",
      context: { decoding, targetFound, rssiSignature: this.rssiSignature },
    });
  }
  ...
  addPacket(packet) {
    ...
    logger.debug({
      message: "addPacket",
      context: { packet, packets: this.packets },
    });
  }
  ...
  addEvent(event) {
    ...
    logger.debug({
      message: "addPacket",
      context: { event, events: this.events },
    });
  }
   ...
   */
  merge(raddec) {
    let isSameTransmitter = identifiers.areMatch(
      this.transmitterId,
      this.transmitterIdType,
    ...
    if (!isSameTransmitter) {
      logger.error({
        message: "Raddec cannot be merged with mismatched transmitter",
        context: { raddec, isSameTransmitter },
      });
  ...
  encodeAsHexString(options) {
    ...
    let transmitterIdentifier =
      toHexString(this.transmitterIdType) + this.transmitterId;
    ...
    logger.info({
      message: "encodeAsHexString",
      context: { options, optionalProperties, raddec, lengthInBytes },
    });
    return raddec;
  }
  ...
  while (hasAdditionalProperty) {
    ...
    switch (propertyFlag) {
      ...
      case 0xf2: // Events
        ...
        break;
      default:
        // Unknown property, exit gracefully
        return;
    }
    hasAdditionalProperty = sourceIndex < (length - 1) * 2;
  }
  logger.info({
    message: "constructFromHexString",
    context: { raddec },
  });
}
...
function constructFromBuffer(instance, source) {
  ...
  logger.debug({
    msg: "constructFromBuffer",
    context: { instance, source, sourceAsHexString },
  });
  constructFromBuffer(instance, sourceAsHexString);
}
...
function constructFromObject(instance, source) {
  ...
  instance.transmitterIdType =
    source.transmitterIdType || identifiers.TYPE_UNKNOWN;
  instance.rssiSignature = [];
  ...
  instance.creationTime = new Date().getTime();
  logger.info({
    msg: "constructFromObject",
    context: { instance, source },
  });
}
...
function mergeTimes(source, destination) {
  if (source.hasOwnProperty("earliestDecodingTime")) {
    if (destination.hasOwnProperty("earliestDecodingTime")) {
      let isSourceEarlier =
        source.earliestDecodingTime < destination.earliestDecodingTime;
      if (isSourceEarlier) {
...
function toHexString(number, numberOfBytes) {
  ...
  logger.debug({
    msg: "toHexString",
    context: { number, numberOfBytes, hexString },
  });
